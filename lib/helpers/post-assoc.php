...
function spectrix_record_inner_custom_box( $post ) {
	...
	if($pagenow == 'post-new.php'){
		...
		wp_nonce_field( plugin_basename( __FILE__ ), 'spectrix_data' );
		...
		echo '</label> ';
		echo '<select id="spectrix_data" name="spectrix_data">';
...
function wpspx_save_postdata($post_id) {
	...
	if ( ! current_user_can( 'edit_post', $post_id ) )
		...
		if ( ! isset( $_POST['spectrix_data'] ) || ! wp_verify_nonce( $_POST['spectrix_data'], plugin_basename( __FILE__ ) ) )
			return;
...
function wpspx_hide_title() {
	...
	if(isset($_GET['post'])) {
		if(get_post_meta($_GET['post'],'_spectrix_id',true)){
